...
const storage = multer.diskStorage({
  ...
  }, limit: 5*1024*1024     // 5MB 제한 = DOS공격 방어
});
...
/////////// 파일 첨부 처리
let imagepath = '';
router.post('/post/photo', upload.single('picture'), (req, res) => {  //text로 들어온걸 binary로 변환,  여러개 받을꺼면 single 대신 array로
  console.log('서버에 파일 첨부하기', req.file.path);
  imagepath = req.file.originalname; // 이미지 경로
});
